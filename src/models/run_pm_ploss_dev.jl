function run_pandamodels_ploss_test(json_path)   # before run_poweramodels
    time_start = time()
    ###############################################################################
    # 0. Initialization
    ###############################################################################
    ...
    JuMP.@objective(model, Min, sum((p[(content["element_index"], content["f_bus"], content["t_bus"])] +
                                    p[(content["element_index"], content["t_bus"], content["f_bus"])])^2
                                    for (i, content) in user_defined_params["target_branch"]))
