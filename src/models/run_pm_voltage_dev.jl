...
function run_pandamodels_vd_test(json_path)   # before run_poweramodels
    time_start = time()
    ###############################################################################
    # 0. Initialization
    ###############################################################################
    ...
    @variable(model, ref[:bus][i]["vmin"] <= vm[i in keys(ref[:bus])] <= ref[:bus][i]["vmax"], start=1.0) #start=ref[:bus][i]["vm"]
    ...
    @objective(model, Min, sum((vm[content["element_index"]] - content["value"])^2
