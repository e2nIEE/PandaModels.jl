function run_pandamodels_vstab_test(json_path)   # before run_poweramodels
    time_start = time()
    ###############################################################################
    # 0. Initialization
    ###############################################################################
    ...
    JuMP.@variable(model, ref[:bus][i]["vmin"] <= vm[i in keys(ref[:bus])] <= ref[:bus][i]["vmax"], start=1.0) #start=ref[:bus][i]["vm"]
    ...
    JuMP.@objective(model, Min, sum((vm[content["element_index"]] - content["value"])^2
                    for (i, content) in user_defined_params["setpoint_v"]))
    ...
    for (i,branch) in ref[:branch]
        ...
        g_to = branch["g_to"]
        b_to = branch["b_to"]
        tm = branch["tap"]^2  # note: tap is assumed to be 1.0 on non-transformer branches
